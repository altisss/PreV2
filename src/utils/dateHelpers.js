...
export const dateWithInputSv = d => moment(d).format('YYYYMMDD');
export const dateWithTime = d => moment(d).format('DD.MM.YYYY HH:mm');
...
export const startOf2Months = moment().subtract(2, 'months').startOf('month');
export const startOf3Months = moment().subtract(3, 'months').startOf('month');
export const startOfHalfYearAgo = moment().subtract(5, 'months').startOf('month');
export const start3YearAgo = moment().subtract(3, 'years').startOf('month');
export const start5YearAgo = moment().subtract(5, 'years').startOf('month');
...
export const formatDateForSubtitle = (d) => {
  if (!d.format) {
    ...
    if (d.from === startOfDay) { return dateWithDots(d.from); }
