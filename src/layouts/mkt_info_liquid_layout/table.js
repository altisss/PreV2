...
const TableHistoryIndex = (props) => {
    ...
    const send2stkinfo = (stk) => {
        ...
        window.ipcRenderer.once(`${commuChanel.reply_get_value_from_glb_sv}_${component}_${sq}`, (event, agrs) => {
            ...
            setTimeout(() => inform_broadcast(commuChanel.misTypeChgStock, msg)
            , 500);
        })
    ...
    const { t } = props;
    // console.log("HHHHHHHHHHHHHHHHH", dataList);
        ...
        <table className="tableNormal table_sticky table table-sm tablenowrap table-bordered table-striped">
            <thead className="header">
                <tr>
                    ...
                    <th style={{ verticalAlign: 'middle' }} className="text-center">
                        {t('priceboard_total_qtty_trading')}{' '}({t('share')})
                        </th>
                    <th style={{ verticalAlign: 'middle' }} className="text-center">
            ...
            <tbody>
                {dataTable.map((item, index) =>
                    <tr key={uniqueId('dataDetails')} className='cursor_ponter' onClick={() => send2stkinfo(item.StkCd)}>
                        <td style={{ verticalAlign: 'middle' }} className={"text-center " + (Number(item.ChgRatio) > 0 ? 'price_basic_over' : (Number(item.ChgRatio) < 0 ? 'price_basic_less' : 'price_basic_color'))}>
                            {item.StkCd}
                        </td>
                        <td style={{ verticalAlign: 'middle' }} className={"text-right " + (Number(item.ChgRatio) > 0 ? 'price_basic_over' : (Number(item.ChgRatio) < 0 ? 'price_basic_less' : 'price_basic_color'))}>
                            {FormatNumber(Number(item.ChgRatio), 2, 0)}%
                          </td>
                        <td style={{ verticalAlign: 'middle' }} className={"text-right " + (getColorPrice(item.LowPri, item))}>
                            {FormatNumber(item.LowPri, 0, 0)}
                        </td>
                        <td style={{ verticalAlign: 'middle' }} className={"text-right " + (getColorPrice(item.HighPri, item))}>
                            {FormatNumber(item.HighPri, 0, 0)}
                        </td>
                        <td style={{ verticalAlign: 'middle' }} className="text-right">
                            {FormatNumber(Number(item.TradVol), 0, 0)}
                        </td>
                        <td style={{ verticalAlign: 'middle' }} className="text-right">
